...
export async function ajouterFigurine(figurineData, imagePrincipale, imagesSupplementaires) {
    try {
        ...
        const figurineExistante = await verifierFigurineExistante(figurineData.codeBarre);
...
async function verifierFigurineExistante(codeBarre) {
    const q = query(collection(db, "figurines"), where("codeBarre", "==", codeBarre));
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (form) {
        ...
        form.addEventListener('submit', async (e) => {
            ...
            const imagePrincipale = formData.get('imagePrincipale');
            const imagesSupplementaires = formData.getAll('imagesSupplementaires');
